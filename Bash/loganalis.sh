...
#Удаляем двойные пробелы (tr), так как они нам будут мешать при парсинге по пробелу. Парсим строку по пробелам и извлекаем поля
# 3, 11, 13 — получаем время, логин и IP, с которого пытаемся ввести пароль неправильно. Все записываем в файл /tmp/secure.
