...
# Если файла pid нет, то сервис не работает. Необходимо его остановить (если он в зависшем состоянии и запустить снова. Также нужно отправить сообщение администратору.
if [ ! -f $pidFile ]
then
